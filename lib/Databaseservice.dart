...
// If the user is the creator, update the creator to the next player.
  Future<String?> getCurrentTurnPlayer(String gameID) async {
    final ref = FirebaseDatabase.instance
        .ref("dice/gameSessions/$gameID/currentPlayer");
    final snapshot = await ref.get();
    return snapshot.value?.toString();
  }

  Future<void> deleteUser(
      String userID, String gameID, String gameChosen) async {
    final ref = await FirebaseDatabase.instance
        .ref("$gameChosen/gameSessions/$gameID/createdBy")
        .once();
    ...
    if (data == userID) {
      ...
      DatabaseReference ref =
          FirebaseDatabase.instance.ref("$gameChosen/gameSessions/$gameID");
      String newCreated = await getNextPlayer(userID, gameID);
      ref.update({"createdBy": newCreated});
      await FirebaseDatabase.instance
          .ref("$gameChosen/gameSessions/$gameID/playersAndDice/$userID")
          .remove();
    } else {
  ...
  Future<void> writeDiceForAll(String userID, String gameID) async {
    ...
    if (data is Map) {
      for (var entry in data.entries) {
        ...
        if (diceList is List) {
          final newRoll =
              List.generate(diceList.length, (_) => rand.nextInt(6) + 1);
...
// Sets the current bet made by a player and transitions the turn
  Future<void> placeDiceBet(
      String userID, String gameID, int amount, int faceValue) async {
    DatabaseReference ref =
        FirebaseDatabase.instance.ref("dice/gameSessions/$gameID");
    await ref.update({
      "betDeclared": [amount, faceValue]
    });
...
// Rotates turn by moving to the next player in the list
  Future<void> setPlayer(String userID, String gameID) async {
    ...
    if (data is Map) {
      ...
      await ref
          .update({"currentPlayer": nextPlayer, "lastPlayer": previousPlayer});
    }
  }
...
// Verifies if a call on a bet was valid by counting matching dice
  Future<bool> checkDiceCall(String userID, String gameID) async {
    ...
    if (data is Map) {
      ...
      if (betDeclare[1] == 1) {
        if (betDeclare[0] == ones) {
          return true;
        }
      }
      if (betDeclare[1] == 2) {
        if (betDeclare[0] == twos) {
          return true;
        }
      }
      if (betDeclare[1] == 3) {
        if (betDeclare[0] == threes) {
          return true;
        }
      }
      if (betDeclare[1] == 4) {
        if (betDeclare[0] == fours) {
          return true;
        }
      }
      if (betDeclare[1] == 5) {
        if (betDeclare[0] == fives) {
          return true;
        }
      }
      if (betDeclare[1] == 6) {
        if (betDeclare[0] == sixs) {
          return true;
        }
      }
      return false;
...
// Retrieves a list of dice values for the current user
  Future<List<int>> getDice(String userID, String gameID) async {
    final data = FirebaseDatabase.instance
        .ref("dice/gameSessions/$gameID/playersAndDice/$userID");
  ...
  Future<int> loseLifeDB(String userId, String gameId) async {
    final ref = FirebaseDatabase.instance
        .ref("dice/gameSessions/$gameId/playersLife/$userId");
    final snapshot = await ref.once();
    final data = snapshot.snapshot.value;
    int playerLives = 0;
    if (data is int) {
  ...
  Future<String> readLastFromLOG(String gameID) async {
    final ref = FirebaseDatabase.instance
        .ref("dice/gameSessions/$gameID/chat")
        .orderByKey()
        .limitToLast(1);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  //Lora's Functions
  Future<void> putDownCardsAndLog(
      String userID, String gameID, List<String> cards) async {
    final DatabaseReference playerRef = FirebaseDatabase.instance
        .ref("deck/gameSessions/$gameID/playersAndCards/$userID");
    final DatabaseReference logRef =
        FirebaseDatabase.instance.ref("deck/gameSessions/$gameID/cardActions");
    final username = await _getUsernameByUid(userID);
...
Future<void> writeUserParameter(
  ...
  final ref = FirebaseDatabase.instance.ref("users/$userId/$parameter");
  await ref.set(value);
}
